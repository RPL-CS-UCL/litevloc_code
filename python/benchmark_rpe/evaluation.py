...
def save_prec_recall_curve(save_dir, precision_curve, recall_curve, eval_config):
	...
	plt.plot(recall_curve, precision_curve, 
			 color='b', lw=2, 
			 label=f'Precision-Recall Curve (RPE)')
	plt.xlim([0.0, 1.0])
	plt.ylim([0.0, 1.05])
	plt.xlabel('Recall')
	...
	curve_path = os.path.join(save_dir, f"prec_recall_curve_{int(config.t_threshold*100)}_{int(config.R_threshold)}.jpg")
...
def main(args):
	...
	if args.split == 'test':
		...
		scenes = tuple(cfg.DATASET.VAL_SCENES)
	try:
	...
	save_prec_recall_curve(os.path.join(args.submission_path, '../'), 
						   curves_data['pose_prec_values'], 
						   curves_data['pose_recall_values'], 
						   args.eval_config)    
	print(output_json)
