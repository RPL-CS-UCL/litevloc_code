...
def main(args):
    ...
    model = initialize_vpr_model(
        args.method, args.backbone, args.descriptors_dimension, args.device
    )

    """Load images"""
    image_graph = ImageGraphLoader.load_data(
        os.path.join(args.dataset_path, "map"),
    ...
    db_descriptors = np.array(
        [map_node.get_descriptor() for _, map_node in image_graph.nodes.items()],
        dtype="float32",
    )
    ...
    obs_poses_gt = np.loadtxt(
        os.path.join(args.dataset_path, "obs", "camera_pose_gt.txt")
    )
    for obs_id in range(0, len(obs_poses_gt), 5):
        ...
        queries_descriptors = np.empty(
            (1, args.vpr_descriptors_dimension), dtype="float32"
        )
